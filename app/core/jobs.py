...
def dummy_number_crunch(job_id: str, job_metadata: dict = None):
    ...
    try:
        ...
        logger.error(f"[{datetime.now(timezone.utc)}] Job {job_id} FAILED: {str(e)}")
        ...
        try:
            with SessionLocal() as db_session:
                ...
                if job:
                    ...
                    db_session.commit()
                    logger.info(f"Job {job_id} marked as FAILED in DB")
        except Exception as inner_e:
            logger.error(f"Failed to mark Job {job_id} as FAILED in DB: {str(inner_e)}")
            logger.error(traceback.format_exc())
